...
def param_shapes(params: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    return jax.tree_map(
        lambda x: list(x.shape), frozen_dict.unfreeze(params)
...
def param_dtypes_shapes(params: Mapping[str, Any]) -> Dict[str, Any]:
    ...
    params = param_remapping.filter_out_metadata(params)
    return jax.tree_map(
        lambda x: [str(x.dtype)] + list(x.shape), frozen_dict.unfreeze(params)
    )  # pytype: disable=wrong-arg-types


def param_dtypes_shapes_axes(
    params: Mapping[str, Any], params_axes: Mapping[str, Any]
) -> Dict[str, Any]:
    ...
    def remove_axes_suffix(ks):
        if not ks[-1].endswith("_axes"):
            raise ValueError(f"Param axes name should end with `_axes`, found {ks[-1]}")
    ...
    def _create_entry(param, param_axes):
        ...
        if param.ndim != len(param_axes.names):
            raise ValueError(
                "Length of param dimension does not match axes, "
