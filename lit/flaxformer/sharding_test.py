...
class ShardingTest(absltest.TestCase):
    ...
    def test_check_params_and_axis_names_match_matches(self):
        sharding.check_params_and_axis_names_match(
            variables={
                "params": {"foo": {"bar": jnp.array([1, 2, 3])}},
                "params_axes": {"foo": {"bar_axes": sharding.axis_names("unsharded")}},
            }
        )
        ...
        with self.assertRaisesRegex(ValueError, ".*foo/bar.*doesn't.*for each"):
            sharding.check_params_and_axis_names_match(
                variables={
                    ...
                    "params_axes": {
                        "foo": {
                            "bar_axes": sharding.axis_names(
                                "unsharded", "model", "vocab"
                            )
                        }
                    },
                }
    ...
    def test_get_axis_names(self):
        variables = {
            ...
            "params_axes": {
                "foo": {"bar_axes": sharding.axis_names("unsharded", "model", "vocab")}
            },
        }
        self.assertEqual(
            sharding.get_axis_names(variables),
            {"foo": {"bar": sharding.AxisNames(("unsharded", "model", "vocab"))}},
        )
