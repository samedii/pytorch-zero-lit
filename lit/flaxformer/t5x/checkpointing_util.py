...
def _apply_remap_fn(
    remap_fn: Callable[[Mapping[str, Any]], Mapping[str, Any]],
...
def make_to_save_format_fn(module: nn.Module) -> checkpoints.SaveStateTransformationFn:
    ...
    if not isinstance(module, param_remapping.ParameterRemappable):
        raise ValueError(
            "Expected `module` to be a `ParameterRemappable`, but was "
            f"{type(module)}"
        )
