    ...
    def get_individual_embeddings(
        ...
        *,
        ...
        if inputs.keys() != self.embedders.keys():
            raise ValueError(
                f"Expected input keys {self.embedders.keys()}, "
        ...
        *,
        ...
        if self.one_hot:
            ...
            if input_axis_names is not None and self.axes:
                one_hot = partitioning.with_sharding_constraint(
                    one_hot, tuple(input_axis_names) + (self.axes[0],)
                )
        ...
        if input_axis_names is not None and self.axes:
            output = partitioning.with_sharding_constraint(
                output, tuple(input_axis_names) + (self.axes[1],)
            )
        return output
...
class MultiEmbed(nn.Module):
    ...
    def get_individual_embeddings(
        ...
        segment_ids: Optional[Array] = None,
        **input_kwargs: Mapping[str, Array],
    ) -> Dict[str, Array]:
        """Embeds each keyword argument with its corresponding embedder.
    ...
    def __call__(
        ...
        **input_kwargs: Mapping[str, Array],
        ...
        if (
            self.sow_intermediates
            and self.capture_gradients
            and self.scope.is_mutable_collection("grads")
        ):
    ...
    def __call__(
        ...
        *,
        ...
        if decode:
            ...
            position_embedder_index.value = i + 1
            return jax.lax.dynamic_slice(
                self.pos_embedding, jnp.array((i, 0)), np.array((1, self.features))
            )
    ...
    def __call__(
        ...
        *,
        ...
        for table, prime in zip(self._tables, self._primes):
            ...
            hash_bucket_ids = ((input_ids + 1) * prime) % self.num_embeddings_per_table
...
class NgramHashEmbed(nn.Module, Embedder[Array]):
    ...
    _tables_by_order: Mapping[str, Sequence[Embed]] = dataclasses.field(init=False)
    ...
    def setup(self):
        ...
        tables_by_order: DefaultDict[int, List[Embed]] = collections.defaultdict(list)
