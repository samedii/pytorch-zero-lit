    ...
    def __call__(self, inputs: Array) -> Array:
        ...
        if self.kernel_axis_names is None:
            ...
            if len(kernel_axis_names) != len(kernel_shape):
                raise ValueError(
                    f"Kernel axis names {kernel_axis_names} doesn't match "
        ...
        out = lax.dot_general(
            inputs, kernel, ((axis, contract_ind), ((), ())), precision=self.precision
        )
        if self.use_bias:
    ...
    def __call__(
        ...
        *,
        ...
        wi_fused_kernel_init = (
            ...
            if self.wi_fused_kernel_init is not None
        ...
        if not self.fuse_kernels:
            ...
            if self.weight_params is not None or self.act_params is not None:
                ...
                raise NotImplementedError(
                    "Quantization is not supported yet for ", "fused kernels."
                )
            if self.precomputed_intermediates:
                if self.out_dim is None:
                    raise ValueError(
                        "Must specify mlp out_dim when using precomputed "
                        "intermediates."
                    )
                xs = inputs
            else:
        ...
        if self.precomputed_intermediates:
            ...
            if self.final_dropout:
                output = self.final_dropout(
                    output, deterministic=not enable_dropout
                )  # pylint: disable=not-callable
            else:
                output = nn.Dropout(rate=self.final_dropout_rate, broadcast_dims=(-2,))(
                    output, deterministic=not enable_dropout
                )
        return output
