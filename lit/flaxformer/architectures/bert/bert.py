...
from lit.flaxformer.architectures.bert import heads
...
from lit.flaxformer.types import Activation
    ...
    def __call__(
        ...
        *batch_sizes, seq_len = input_ids.shape
        ...
        result = self.encoder_block(
            embeddings, attention_mask=attention_mask, enable_dropout=enable_dropout
        )
...
class EncoderBlock(nn.Module):
    ...
    def __call__(
        self, inputs: Array, *, attention_mask: Array, enable_dropout: bool = True
    ) -> Array:
    ...
    def __call__(
        ...
        *,
        ...
        result = self.attention_block(
            inputs=inputs,
            attention_targets=(
                attention_targets if attention_targets is not None else inputs
            ),
            mask=attention_mask,
            bias=attention_bias,
            enable_dropout=enable_dropout,
...
class AttentionBlock(nn.Module):
    ...
    def __call__(
        ...
        *batch_sizes, seq_len, features = inputs.shape
        ...
        if mask is not None:
            chex.assert_shape(
                mask, (*batch_sizes, {num_heads, 1}, seq_len, target_seq_len)
            )
        if bias is not None:
            chex.assert_shape(
                bias, (*batch_sizes, {num_heads, 1}, seq_len, target_seq_len)
            )
...
def make_full_encoder(
    ...
    return FullEncoder(
        embedder_block=EmbedderBlock(
            embedder=embedding.MultiEmbed(
                {
                    "input_ids": embedding.Embed(
                        num_embeddings=vocab_size,
                        features=hidden_size,
                        embedding_init=kernel_init,
                    ),
                    "position_ids": embedding.Embed(
                        num_embeddings=max_length,
                        features=hidden_size,
                        embedding_init=kernel_init,
                    ),
                    "segment_ids": embedding.Embed(
...
def make_encoder_block(
    *,
    ...
    return EncoderBlock(
        common.LayerSequence(
            num_layers=num_layers, make_layer=make_layer
...
class BertEncoder(nn.Module):
    ...
    dropout_rate: float = 0.0
    dtype: jnp.dtype = jnp.float32
    kernel_init: Initializer = initializers.truncated_normal(stddev=_DEFAULT_INIT_RANGE)
    bias_init: Initializer = nn.initializers.zeros
    layer_norm_epsilon: float = _DEFAULT_LAYER_NORM
    ...
    def embed_and_combine_inputs(
        self, token_ids: Array, position_ids: Array, segment_ids: Array
    ) -> Array:
    ...
    def finalize_embeddings(
        self, embedded_inputs: Array, *, enable_dropout: Optional[bool] = None
    ) -> Array:
        """Finalize embedded inputs to be sent to the first transformer layer."""
        enable_dropout = nn.module.merge_param(
            "enable_dropout", self.enable_dropout, enable_dropout
        )
