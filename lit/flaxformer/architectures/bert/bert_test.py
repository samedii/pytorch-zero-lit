...
from lit.flaxformer.architectures.bert import bert
from lit.flaxformer.architectures.bert import configs as bert_configs
...
class BertMlmNspTest(absltest.TestCase):
    ...
    def test_params(self):
        ...
        expected_param_shapes = {
            ...
            "mlm_head": {
                "bias": (10,),
                "dense": {"bias": (4,), "kernel": (4, 4)},
                "layer_norm": {"bias": (4,), "scale": (4,)},
            },
            "nsp_head": {"dense": {"bias": (2,), "kernel": (4, 2)}},
            "pooler": {"dense": {"bias": (4,), "kernel": (4, 4)}},
        }
...
class BertClassifierTest(absltest.TestCase):
    def test_output_shapes(self):
        ...
        pooler = heads.BertPooler()
        classifier_head = heads.ClassifierHead(pooler=pooler, num_classes=num_classes)
...
class EncoderLayerTest(absltest.TestCase):
    def test_wrong_inputs_dimension(self):
        ...
        encoder_layer = bert.make_encoder_layer(
            bert.make_attention_layer(num_heads=2), hidden_size=4, intermediate_dim=14
        )
...
class AttentionBlockTest(absltest.TestCase):
    def test_output_shape(self):
        ...
        self.assertDictEqual(
            testing_utils.param_dtypes_shapes_axes(params, variables["params_axes"]),
            {
                "attention_layer": {
                    "key": {
...
class MlpBlockTest(absltest.TestCase):
    def test_output_shape(self):
        ...
        result, variables = layer.init_with_output(
            {"params": params_key, "dropout": dropout_key}, inputs, enable_dropout=False
        )
        self.assertEqual(input_shape, result.shape)
