...
class MLMHead(nn.Module):
    ...
    def setup(self):
        ...
        self.layer_norm = nn.LayerNorm(epsilon=_DEFAULT_LAYER_NORM, name="layer_norm")
        self.bias = self.param("bias", zeros, (self.vocab_size,))

    def __call__(
        self, encoded_inputs: Array, *, masked_positions: Optional[Array]
    ) -> Array:
...
class MLP(nn.Module):
    ...
    @nn.compact
    def __call__(
        self, inputs: Array, *, enable_dropout: Optional[bool] = None, **unused_kwargs
    ) -> Array:
        ...
        if activations is None:
            ...
            raise ValueError(
                "`activations` must be of length `len(features) - 1`. "
