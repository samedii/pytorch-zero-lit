...
from lit.flaxformer import param_conversion_util
...
def load_params_from_tf_checkpoint(
    ...
    pooler_params = param_conversion_util.convert_tf_params(
        tf_params,
        {("dense", "bias"): "bias", ("dense", "kernel"): "kernel"},
        "bert/pooler/dense/",
    )
...
def convert_bert_encoder_params(
    tf_params: Dict[str, tf.Tensor], prefix: str
) -> Dict[str, Any]:
    """Loads all Flax BertEncoder parameters from the TF params."""
    return {
        "embedder": param_conversion_util.convert_tf_params(
            tf_params,
            {
                ("embedders_position_ids", "embedding"): "position_embeddings",
                ("embedders_segment_ids", "embedding"): "token_type_embeddings",
                ("embedders_token_ids", "embedding"): "word_embeddings",
            },
            f"{prefix}embeddings/",
        ),
        "layer_norm": convert_layer_norm_params(
            tf_params, f"{prefix}embeddings/LayerNorm/"
        ),
        "encoder_block": convert_encoder_block_params(tf_params, f"{prefix}encoder/"),
    }


def convert_full_encoder_params(
    tf_params: Dict[str, tf.Tensor], prefix: str
) -> Dict[str, Any]:
    """Loads all Flax Encoder parameters from the TF params."""
    return {
        "embedder_block": convert_embedder_block_params(
            tf_params, f"{prefix}embeddings/"
        ),
        "encoder_block": convert_encoder_block_params(tf_params, f"{prefix}encoder/"),
    }


def convert_embedder_block_params(
    tf_params: Dict[str, tf.Tensor], prefix: str
) -> Dict[str, Any]:
...
def convert_encoder_block_params(
    tf_params: Dict[str, tf.Tensor], prefix: str
) -> Dict[str, Any]:
    ...
    for i in layer_indices:
        layers[f"layers_{i}"] = convert_encoder_layer_params(
            tf_params, f"{prefix}layer_{i}/"
        )
    return {"layer_sequence": layers}


def convert_encoder_layer_params(
    tf_params: Dict[str, tf.Tensor], prefix: str
) -> Dict[str, Any]:
    """Loads all Flax EncoderLayer parameters from the TF params."""
    return {
        "attention_block": convert_self_attention_block_params(
            tf_params, f"{prefix}attention/"
        ),
        "mlp_block": convert_mlp_block_params(tf_params, prefix),
    }


def convert_self_attention_block_params(
    tf_params: Dict[str, tf.Tensor], prefix: str
) -> Dict[str, Any]:
    """Loads all Flax AttentionBlock parameters from the TF params."""
    return {
        "attention_layer": convert_attention_layer_params(tf_params, f"{prefix}self/"),
        "dense_layer": convert_dense_layer_params(tf_params, f"{prefix}output/dense/"),
        "layer_norm": convert_layer_norm_params(
            tf_params, f"{prefix}output/LayerNorm/"
        ),
    }


def convert_mlp_block_params(
    tf_params: Dict[str, tf.Tensor], prefix: str
) -> Dict[str, Any]:
    """Loads all Flax MlpBlock parameters from the TF params."""
    return {
        "mlp": {
            "dense_layer": convert_dense_layer_params(
                tf_params, f"{prefix}intermediate/dense/"
            ),
        },
        "dense_layer": convert_dense_layer_params(tf_params, f"{prefix}output/dense/"),
        "layer_norm": convert_layer_norm_params(
...
def convert_attention_layer_params(
    tf_params: Dict[str, tf.Tensor], prefix: str
) -> Dict[str, Any]:
...
def convert_dense_layer_params(
    tf_params: Dict[str, tf.Tensor], prefix: str
) -> Dict[str, Any]:
