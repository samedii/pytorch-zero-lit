...
def _take_along_axis(array: Array, indices: Array, axis: int) -> Array:
    ...
    if (
        axis != -1
        and axis != array.ndim - 1
        and axis != 1  # Not last dimension
        and axis != -array.ndim + 1
...
class Router(nn.Module):
    ...
    def __call__(
        ...
        instructions = self._compute_routing_instructions(
            router_probs, padding_mask, expert_capacity
        )
...
class TokensChooseScatterRouter(ScatterRouter):
    ...
    def _compute_routing_instructions(
        ...
        preferred_experts = preferred_experts.reshape(
            num_groups, self.num_selected_experts, tokens_per_group
        )
        # Shape: [num_groups, tokens_per_group, num_selected_experts]
    ...
    def _compute_routing_instructions(
        ...
        if padding_mask is not None:
            ...
            expert_index += jnp.repeat(
                gate_mask - 1.0, self.num_selected_experts, axis=-1
            )
