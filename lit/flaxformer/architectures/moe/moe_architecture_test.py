...
from lit.flaxformer.architectures.moe import moe_architecture
from lit.flaxformer.architectures.moe import moe_enums
from lit.flaxformer.architectures.moe import moe_layers
from lit.flaxformer.architectures.moe import routing
...
def _make_multi_query_attention() -> dense_attention.MultiQueryDotProductAttention:
...
def _make_relative_position_bias() -> relative_position_biases.RelativePositionBiases:
...
def _make_dense_encoder_layer(
    shared_relative_position_bias: Optional[nn.Module] = None, scanned: bool = False
) -> EncoderLayer:
...
class MoeArchitectureTest(parameterized.TestCase):
    ...
    def test_scan_architecture(self):
        ...
        encoder_factory = functools.partial(
            ...
            layer_factory=functools.partial(_make_dense_encoder_layer, scanned=False),
            input_dropout_factory=_make_dropout,
            output_dropout_factory=_make_dropout,
            layer_norm_factory=_make_layer_norm,
        )
        decoder_factory = functools.partial(
            ...
            layer_factory=functools.partial(_make_dense_decoder_layer, scanned=False),
            dropout_factory=_make_dropout,
            layer_norm_factory=_make_layer_norm,
        )
        ...
        with self.subTest(name="sow_intermediates"):
            ...
            for metric in MOE_METRICS:
                self.assertIn(
                    metric, intermediates["encoder"]["encoder"]["subblock_1"]["mlp"]
                )
                self.assertIn(
                    metric, intermediates["decoder"]["decoder"]["subblock_0"]["mlp"]
                )
    ...
    def test_sparse_layouts(
        ...
        for layer in expected_sparse_layers:
            self.assertTrue(
                moe_architecture._is_sparse_layer(
                    layer, num_layers, num_sparse_layers, sparse_layout
                )
            )
        ...
        with self.subTest(name="unsupported_layouts"):
            for layer_layout in [
                ...
                with self.assertRaisesRegex(
                    ValueError, "Scan is only supported for MIXED"
                ):
                    scan_block_factory(
    ...
    def test_num_scan_blocks(self):
        self.assertEqual(
            moe_architecture._num_scan_blocks(
                num_layers=24, num_sparse_layers=3, sparse_layout=LayerLayout.MIXED
            ),
            3,
        )
