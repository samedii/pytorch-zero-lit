...
from lit.flaxformer.architectures.moe import moe_parallel_fused_decoder
...
def _make_o_wo_fused_projection(
    ...
    expert = dense.DenseGeneral(
        axis=(-2, -1),
        features=moe_parallel_fused_decoder.compute_fused_o_wo_dims(attention_module),
        use_bias=False,
        dtype=DTYPE,
        kernel_init=ATTENTION_KERNEL_INIT,
        bias_init=BIAS_INIT,
...
def _make_kv_fused_projection(
    ...
    return dense.DenseGeneral(
        axis=-1,
        features=moe_parallel_fused_decoder.compute_fused_kv_dims(attention_module),
        use_bias=False,
        dtype=DTYPE,
        kernel_init=ATTENTION_KERNEL_INIT,
        bias_init=BIAS_INIT,
...
class ParallelFusedDecoderOnlyTest(absltest.TestCase):
    def test_layer(self):
        ...
        attention_module = _make_multi_query_attention(embed_dim, num_heads, head_dim)
