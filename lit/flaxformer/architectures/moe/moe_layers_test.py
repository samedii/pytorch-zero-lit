...
class MoeLayerTest(parameterized.TestCase):
    ...
    def test_moe_layer_runs(self, dispatch: str):
        ...
        init_batch = {
            "inputs": jnp.ones((batch_size, max_seq_length, hidden_dim), jnp.float32)
        }
        ...
        inputs = jax.random.uniform(
            init_rng, (batch_size, max_seq_length, hidden_dim), minval=-10, maxval=10
        )
    ...
    def test_dense_general_expert(self):
        ...
        init_batch = {
            "inputs": jnp.ones((batch_size, max_seq_length, *hidden_dims), jnp.float32)
        }
        params = init_layer_variables(rng, moe_layer, init_batch)["params"]
        ...
        dropout_rng, jitter_rng, init_rng = jax.random.split(rng, num=3)
        inputs = jax.random.uniform(
            init_rng, (batch_size, max_seq_length, *hidden_dims), minval=-10, maxval=10
        )
    ...
    def test_num_partitions_incorrect(self, num_model_partitions: int):
        ...
        with self.assertRaisesRegex(
            ValueError,
            f"optimize_model_parallel_communications=True with "
