...
from lit.flaxformer import transformer_common as common
from lit.flaxformer.architectures.common import param_remapping
from lit.flaxformer.architectures.moe import moe_enums
from lit.flaxformer.architectures.moe import moe_layers
...
class SparseEncoderLayer(EncoderLayer):
    ...
    def __call__(
        ...
        *,
        ...
        """
        y = super().__call__(
            inputs, encoder_mask, logit_mask=logit_mask, enable_dropout=enable_dropout
        )
        ...
        z = self.extra_mlp(
            z, enable_dropout=enable_dropout
...
class MoeEncoderScanBlock(nn.Module, param_remapping.ParameterRemappable):
    ...
    def setup(self) -> None:
        self.subblock: Sequence[EncoderLayer] = _scan_block_factory(
            self.dense_layer_factory,
            self.sparse_layer_factory,
            self.num_layers,
...
class SparseEncoder(t5_architecture.Encoder):
    ...
    def setup(self):
        _validate_module_construction(self.sparse_layer_factory, self.num_sparse_layers)
        ...
        dense_layer_factory = (
            lambda: self.layer_factory(  # pylint:disable=g-long-lambda
                shared_relative_position_bias=self.relpos_bias
            )
        )
        sparse_layer_factory = (
            lambda: self.sparse_layer_factory(  # pylint:disable=g-long-lambda
                shared_relative_position_bias=self.relpos_bias
            )
        )
        ...
        if not self.scan_layers:
            ...
            block_factory = t5_architecture.maybe_remat(
                block_factory, self.layer_remat, self.scan_layers, static_argnums=(3,)
            )
            self.encoder = self._construct_scanned_encoder(
                block_factory,
                num_layers=_num_scan_blocks(
                    self.num_layers, self.num_sparse_layers, self.sparse_layout
                ),
            )
...
def _validate_module_construction(
    sparse_layer_factory: Union[
        Optional[MakeEncoderLayerFn], Optional[MakeDecoderLayerFn]
    ],
    ...
    if sparse_layer_factory is None:
        raise ValueError("sparse_layer_factory must be specified but was left as None.")
    if num_sparse_layers is None:
        raise ValueError("num_sparse_layers must be specified but was left as None.")


def _is_sparse_layer(
    layer: int, num_layers: int, num_sparse_layers: int, sparse_layout: LayerLayout
) -> bool:
    ...
    if sparse_layout == LayerLayout.BOTTOM:
        ...
        return (
            num_layers - num_sparse_layers <= 2 * layer < num_layers + num_sparse_layers
        )
...
def _layer_factory(
    layer: int,
    dense_layer_factory: Callable[[], T],
    sparse_layer_factory: Callable[[], T],
    num_layers: int,
    num_sparse_layers: int,
    sparse_layout: Callable[[], T],
) -> T:
...
def _num_scan_blocks(
    num_layers: int, num_sparse_layers: int, sparse_layout: LayerLayout
) -> int:
