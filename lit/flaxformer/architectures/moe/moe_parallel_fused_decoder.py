...
class SparseParallelFusedDecoderLayer(nn.Module, param_remapping.ParameterRemappable):
    """Sparse parallel Transformer decoder layer with fused parameters.
    ...
    def __call__(
        ...
        *,
        ...
        x = self.layer_norm(
            layer_input, decode=decode, prefill=prefill, prefill_lengths=prefill_lengths
        )
        ...
        # Slice out value.
        value = jnp.squeeze(lax.dynamic_slice_in_dim(kv, head_dim, head_dim, -1), -2)
...
def compute_fused_q_wi_dims(
    attention_module: dense_attention.MultiQueryDotProductAttention, mlp: dense.MlpBlock
) -> Tuple[int, int]:
    ...
    return (num_heads, (mlp_intermediate_dim // num_heads) * n_activations + head_dim)
