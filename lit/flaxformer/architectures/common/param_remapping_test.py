...
class NewDense(nn.Module, param_remapping.ParameterRemappable):
    ...
    def _from_save_format(
        self, params: param_remapping.RecursiveDefaultDict
    ) -> Mapping[str, Any]:
...
class ParameterRemappableTest(parameterized.TestCase):
    @parameterized.named_parameters(
        [
            ("direct_init", Mlp([NewDense(shape=(3, 4)), NewDense(shape=(5, 3))])),
    ...
    def test_load_old_checkpoint(self, new_mlp):
        ...
        self.assertSameStructure(
            ...
            "old_mlp_vars = "
            + testing_utils.format_params_shapes(
                testing_utils.param_shapes(old_mlp_vars)
            ),
        )
        ...
        self.assertSameStructure(
            ...
            {
                "params": {
                    "layers_0": {"weights": [3, 4]},
                    "layers_1": {"weights": [5, 3]},
                }
            },
            "new_mlp_remapped_vars = "
            + testing_utils.format_params_shapes(
    ...
    def test_nested_structures(self):
        old_mlp = NestedStructuresMlp(
            [
                [OldDense(shape=(3, 4)), OldDense(shape=(5, 3))],
                [OldDense(shape=(6, 5)), OldDense(shape=(7, 6))],
            ]
        )
        old_mlp_vars = old_mlp.init(jax.random.PRNGKey(0), jnp.zeros([4]))
        self.assertSameStructure(
        ...
        new_mlp = NestedStructuresMlp(
            [
                [NewDense(shape=(3, 4)), NewDense(shape=(5, 3))],
                [NewDense(shape=(6, 5)), NewDense(shape=(7, 6))],
            ]
        )
        ...
        self.assertSameStructure(
            ...
            {
                "params": {
                    "layers_0_0": {"weights": [3, 4]},
                    "layers_0_1": {"weights": [5, 3]},
                    "layers_1_0": {"weights": [6, 5]},
                    "layers_1_1": {"weights": [7, 6]},
                }
            },
            "new_mlp_remapped_vars = "
            + testing_utils.format_params_shapes(
