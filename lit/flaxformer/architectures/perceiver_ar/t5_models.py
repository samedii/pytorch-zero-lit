...
import seqio
from lit.t5x import decoding
from lit.t5x import losses
from lit.t5x import models
...
def _crop_sequences(sequences: jnp.ndarray, lengths: jnp.ndarray) -> jnp.ndarray:
...
def crop_train_batch(
    ...
    last_loss_idx = (
        batch["decoder_loss_weights"].shape[-1]
        - 1
        - jnp.argmax(jnp.flip(batch["decoder_loss_weights"] == 1, axis=-1), axis=-1)
    )
    ...
    seq_crop_first_idx = jax.random.randint(
        rng,
        [batch["decoder_loss_weights"].shape[0]],
        min_crop_start,
        max_crop_start + 1,
    )

    seq_crop_end = jnp.minimum(seq_crop_first_idx + num_latents, last_loss_idx + 1)
...
class PerceiverARModel(models.DecoderOnlyModel):
    ...
    def __init__(
        ...
        if decoding_latent_reset_fill:
            ...
            self._decoding_latent_reset_fill = max(num_latents - 128, num_latents // 2)
    ...
    def loss_fn(
        ...
        if dropout_rng is None:
            ...
            logging.info(
                ...
                "Should happen only during eval.",
                self._train_cropping_method,
            )
        else:
        ...
        targets = jax.vmap(
            functools.partial(
                lax.dynamic_slice_in_dim, slice_size=self._num_latents, axis=-1
            )
        )(batch["decoder_target_tokens"], q_start)
    ...
    def _compute_logits_from_slice(
        ...
        logging.info(
            "Using a reset step latent fill of %d positions",
        ...
        def reset_step():
            ...
            reset_cache = tree_map_self_att_cache(
                prepare_reset_cache, decoding_state.cache
            )
            ...
            flat_logits = jax.vmap(
                functools.partial(lax.dynamic_slice_in_dim, slice_size=1, axis=-2)
            )(logits, logits_idx)
        ...
        for layer_name, layer_cache in get_cache_by_layers(
            ...
            if "layers_0" in layer_name:
                continue
            needs_reset |= (
                layer_cache["cache_index"] >= layer_cache["cached_key"].shape[-1]
            ).any()

        flat_logits, new_flat_cache = lax.cond(needs_reset, reset_step, regular_step)
    ...
    def predict_batch_with_aux(
        ...
        *,
        ...
        prefilled_cache = self._compute_kv_cache(
            params, inputs, inputs_lengths, batch["decoder_causal_attention"]
        )
    ...
    def score_batch(
        ...
        def body(state):
            ...
            loop_logits = jnp.pad(
                loop_logits, [(0, 0), (0, input_length - self._num_latents), (0, 0)]
            )
            loop_logits_shift = slice_end - self._num_latents
            loop_logits = jax.vmap(functools.partial(jnp.roll, axis=0))(
                loop_logits, loop_logits_shift
            )
            ...
            logits = jnp.where(
                ...
                >= state["slice_end"][:, jnp.newaxis, jnp.newaxis],
                loop_logits,
                state["logits"],
            )
        ...
        final_state = lax.while_loop(cond_fun=cond, body_fun=body, init_val=init_state)
