...
from lit.flaxformer.architectures.perceiver_ar import attention
from lit.flaxformer.architectures.perceiver_ar import slicing
from lit.flaxformer.architectures.t5 import parallel_fused_decoder
    ...
    def __call__(
        ...
        *,
        ...
        for layer in self.layers:
            ...
            if (
                layer_decoder_mask is not None
                and layer_decoder_mask.shape[-1] != current_activations.shape[-2]
            ):
            ...
            if prefill:
                ...
                layer_prefill_lengths = jnp.minimum(
                    current_activations.shape[-2], layer_prefill_lengths
                )

            layer_logit_mask = logit_mask
            if (
                layer_logit_mask is not None
                and layer_logit_mask.shape[-2] != current_activations.shape[-2]
            ):
...
class Decoder(t5_architecture.Decoder):
    ...
    def _setup_layer_sequence(self):
        ...
        lyrf = t5_architecture.maybe_remat(
            lyrf, self.layer_remat, self.scan_layers, static_argnums=(5, 6, 7, 8, 9, 10)
        )
        ...
        if not self.scan_layers:
            ...
            lyrf_notscanned = lambda: self.layer_factory(  # pylint: disable=g-long-lambda  # pytype: disable=wrong-keyword-args
                shared_relative_position_bias=self.relpos_bias, scanned=False
            )
...
class DecoderOnly(t5_architecture.DecoderOnly):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        sequence_lengths = (decoder_target_tokens > 0).astype(jnp.int32).sum(axis=-1)
