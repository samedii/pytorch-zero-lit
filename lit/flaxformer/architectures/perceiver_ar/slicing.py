...
def _slice_sequences(
    x: Array, sequence_length: Array, num_latents: int, axis: int
) -> Array:
...
def slice_sequences_vmap(
    x: Array, sequence_lengths: Array, num_latents: int, axis_within_vmap: int
) -> Array:
    ...
    return jax.vmap(
        functools.partial(
            _slice_sequences, num_latents=num_latents, axis=axis_within_vmap
        )
    )(x, sequence_lengths)


def slice_sequences_xmap(
    x: Array, sequence_lengths: Array, num_latents: int, axis_within_xmap: int
) -> Array:
    ...
    Returns:
      Sliced input array.
    """
    if jax.devices()[0].platform != "cpu" and partitioning.global_mesh_defined():
    ...
    return maps.xmap(
        ...
        axis_resources=xmap_axis_resources,
    )(x, sequence_lengths)
