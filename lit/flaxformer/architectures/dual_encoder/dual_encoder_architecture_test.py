...
from lit.flaxformer.architectures.dual_encoder import (
    dual_encoder_architecture,
)
from lit.flaxformer.architectures.dual_encoder import l2_norm
from lit.flaxformer.architectures.dual_encoder import poolings
from lit.flaxformer.architectures.dual_encoder import (
    similarity_functions,
)
from lit.flaxformer.architectures.t5 import (
    t5_architecture as flaxformer_t5_architecture,
)
...
class NonRepeatingDropoutTest(parameterized.TestCase):
    def test_non_repeating_dropout(self):
        ...
        inputs_diff, _ = make_test_call_twice_dropout().apply(
            {}, inputs, rngs={"dropout": rnd_seed}, mutable="dropout"
        )  # have to increment the counters
...
class DualEncoderTest(parameterized.TestCase):
    ...
    def test_dual_encoder_with_batch_dot_product_shapes(self, pool_method):
        ...
        reformatted = model.apply({}, variables["params"], method=model.to_save_format)
        if pool_method == "attention":
            ...
            check_dual_encoder_params(
                reformatted, "dual_encoder_shapes_batch_dot_product.json"
            )
    ...
    def test_dual_encoder_with_batch_dot_product_negative_shapes(self):
        ...
        self.assertEqual(
            logits.shape, (left_batch_size, right_batch_size + negative_batch_size)
        )
    ...
    def test_dual_encoder_with_pointwise_ffnn_shapes(self):
        ...
        reformatted = model.apply({}, variables["params"], method=model.to_save_format)
        check_dual_encoder_params(
            reformatted, "dual_encoder_shapes_pointwise_ffnn.json"
        )
