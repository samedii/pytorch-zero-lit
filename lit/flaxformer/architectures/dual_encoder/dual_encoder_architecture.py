    ...
    def __call__(self, inputs, deterministic: Optional[bool] = None, rng=None):
        ...
        if deterministic:
            ...
            if rng is None:
                ...
                cntr = self.variable(
                    "dropout", "counter", lambda: jnp.array(0)
                )  # <--- autoincrements dropout
                rng = random.fold_in(
                    rng, cntr.value
                )  # requires that we set mutable='dropout' in apply
                cntr.value += 1  #
    ...
    def setup(self):
        ...
        if self.projection_layer_factory:
            self.projection_layer = (
                self.projection_layer_factory()
            )  # pylint: disable=not-callable

        if self.similarity_layer_factory:
            self.similarity_layer = (
                self.similarity_layer_factory()
            )  # pylint: disable=not-callable
