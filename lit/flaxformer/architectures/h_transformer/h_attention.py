...
import numpy as np
from lit.flaxformer import activation_partitioning
from lit.flaxformer.architectures.h_transformer import (
    hierarchical_relative_position_bias as h_rpb,
)
from lit.flaxformer.architectures.h_transformer import (
    token_hierarchy as th,
)
...
class HierarchicalAttention(nn.Module, metaclass=abc.ABCMeta):
    ...
    def __call__(
        ...
        """
        self._validate_call_parameters(
            inputs_q, inputs_kv, query_padding_mask, key_padding_mask
        )
        if (
            query_padding_mask is not None
            and query_padding_mask.ndim == inputs_q.ndim - 1
        ):
        ...
        for block_name in hierarchy.block_names:
            query[block_name] = dense_q(coarse_qkv[th.InputArrayName.QUERY][block_name])
            key[block_name] = dense_k(coarse_qkv[th.InputArrayName.KEY][block_name])
            value[block_name] = dense_v(coarse_qkv[th.InputArrayName.VALUE][block_name])
    ...
    def _validate_call_parameters(
        ...
        def _validate_padding_mask_shape(
            padding_mask: Array, inputs: Array, mask_name: str
        ):
            expected_shape = (
                inputs.shape[:-1]
                if padding_mask.ndim == inputs.ndim - 1
                else inputs.shape[:-1] + (1,)
            )
        ...
        if query_padding_mask is not None:
            _validate_padding_mask_shape(
                query_padding_mask, inputs_q, "query_padding_mask"
            )
        if key_padding_mask is not None:
            _validate_padding_mask_shape(
                key_padding_mask, inputs_kv, "key_padding_mask"
            )
        ...
        if inputs_q.ndim != inputs_kv.ndim:
            raise ValueError(
                f"Mismatched inputs rank: expected "
        ...
        if inputs_q.shape[:-1] != inputs_kv.shape[:-1]:
            raise ValueError(
                f"Mismatched inputs_kv and inputs_q shape: expected "
    ...
    def _setup_hierarchy(
        self, features: int, for_self_attention: bool
    ) -> th.TokenHierarchy:
    ...
    @abc.abstractmethod
    def _setup_position_bias(self, hierarchy) -> h_rpb.HierarchicalRelativePositionBias:
    ...
    def _hierarchical_attention_fn(
        ...
        aggregated_key_padding_mask: Optional[Dict[th.TokenBlockName, Array]] = None,
    ...
    def _find_max_similarity(
        self, similarity: Dict[th.TokenBlockName, Array], hierarchy: th.TokenHierarchy
    ) -> Array:
    ...
    def _gen_correction_mask(
        self, hierarchy: th.TokenHierarchy
    ) -> Dict[th.TokenBlockName, Array]:
    ...
    def _compute_softmax_partition(
        ...
        head_dim: int,
        query_padding_mask: Optional[Array] = None,
        aggregated_key_padding_mask: Optional[Array] = None,
    ) -> Array:
    ...
    def _column_sum(
        self, attention: Dict[th.TokenBlockName, Array], hierarchy: th.TokenHierarchy
    ) -> Array:
        ...
        Returns:
          softmax_partition: Partition for the softmax calculation. Array shape
            is [batch, length, 1].
        """
        zero_block_mask = hierarchy.gen_packed_zero_block_mask(use_growth_factor=True)
        first_block = True
        for block_name in hierarchy.block_names:
    ...
    def _attention_dropout(
        self, attention: Array, hierarchy: th.TokenHierarchy, dropout_rng: PRNGKey
    ) -> Array:
        ...
        for block_name in hierarchy.block_names:
            attention[block_name] *= dropout_multiplier(
                attention[block_name], dropout_rng
            )
        return attention
...
class OneDimHierarchicalAttention(HierarchicalAttention):
    ...
    def _gen_correction_mask(
        ...
        return {
            th.TokenBlockName.RIGHT: jnp.concatenate(
                (all_ones_block, right_blocks), axis=0
            ),
            th.TokenBlockName.LEFT: jnp.concatenate(
                (all_ones_block, left_blocks), axis=0
            ),
        }
    ...
    def _gen_causal_mask(self, hierarchy: th.TokenHierarchy) -> Array:
        ...
        causal_mask = jnp.tril(
            jnp.ones((hierarchy.num_cluster, hierarchy.num_cluster), dtype=self.dtype),
            k=0,
        )
    ...
    def _duplicate_heads(self, softmax_partition: Array, head_dim: int) -> Array:
        ...
        Returns:
          New softmax_partition with added duplicated entries.
        """
        softmax_partition = jnp.repeat(softmax_partition[..., None], head_dim, axis=3)
...
class OneDimDecoderSelfAttention(OneDimHierarchicalAttention):
    ...
    def __call__(
        self, inputs: Array, padding_mask: Array, enable_dropout: Optional[bool] = False
    ) -> Array:
