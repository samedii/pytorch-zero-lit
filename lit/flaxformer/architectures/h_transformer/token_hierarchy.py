...
class OneDimTokenCoarsening:
    ...
    def __init__(
        ...
        if method == TokenCoarseningMethod.SAMPLE:
            ...
            const_entry = 1.0 / conv_kernel_size
...
class TokenHierarchy(metaclass=abc.ABCMeta):
    ...
    def gen_packed_zero_block_mask(
        self, use_growth_factor: bool = False
    ...
    @abc.abstractmethod
    def _gen_zero_block_mask(self, level: int, block_name: TokenBlockName) -> Array:
...
class OneDimTokenHierarchy(TokenHierarchy):
    ...
    def hierarchical_coarsen(
        ...
        packed_coarse_qkv = {
            InputArrayName.QUERY: self._pack_coarse_qkv(
                coarse_q, input_name=InputArrayName.QUERY
            ),
            InputArrayName.KEY: self._pack_coarse_qkv(
                coarse_k, input_name=InputArrayName.KEY
            ),
            InputArrayName.VALUE: self._pack_coarse_qkv(
                coarse_v, input_name=InputArrayName.VALUE
            ),
        }
    ...
    def _coarsen_query_or_key(
        ...
        need_aggregation: bool = False,
    ) -> CoarsenQueryOrKeyResults:
            ...
            if block_name == TokenBlockName.ANCHOR:
                ...
                for level in range(self.num_level):
                    if to_replace:
                        ...
                        packed_list.append(
                            _shift_blocks_1d(coarse_qkv[level], block_name)
                        )
