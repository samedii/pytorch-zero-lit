...
from lit.flaxformer.architectures.h_transformer import h_attention
from lit.flaxformer.architectures.h_transformer import (
    h_transformer_1d_architecture,
)
from lit.flaxformer.architectures.h_transformer import (
...
_ATTENTION_KERNEL_INIT = nn.initializers.variance_scaling(1.0, "fan_in", "normal")
_MLP_KERNEL_INIT = nn.initializers.variance_scaling(1.0, "fan_in", "truncated_normal")
...
def config_encoder(
    ...
    dropout_factory: Callable[..., nn.Module] = lambda: nn.Dropout(
        rate=0.1, broadcast_dims=(-2,)
    ),
    num_layers: int = 3,
    vocab_size: int = 2000,
    ...
    def _encoder_layer_factory():
        return h_transformer_1d_architecture.EncoderLayer(
            attention=_encoder_self_attention_factory(
                num_heads, num_clusters, qkv_features
            ),
...
def config_decoder(
    ...
    def _decoder_layer_factory():
        return h_transformer_1d_architecture.DecoderLayer(
            self_attention=_decoder_self_attention_factory(
                num_heads, num_clusters, qkv_features
            ),
...
def config_encoder_decoder(
    ...
    def _decoder_layer_factory():
        return h_transformer_1d_architecture.DecoderLayer(
            ...
            encoder_decoder_attention=_cross_attention_factory(num_heads, qkv_features),
            mlp=_mlp_factory(dropout_rate=dropout_rate, embed_size=embed_size),
            dropout_factory=dropout_factory,
            layer_norm_factory=layer_norm_factory,
            scanned=scan_layers,
