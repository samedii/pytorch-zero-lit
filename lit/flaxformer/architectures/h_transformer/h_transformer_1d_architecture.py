...
def _activation_partitioning_fn(y):
    return nn.partitioning.with_sharding_constraint(y, ("batch", "length", "embed"))
...
class EncoderLayer(nn.Module, param_remapping.ParameterRemappable):
    ...
    def __call__(
        self, inputs: Array, inputs_mask: Array, *, enable_dropout: bool = True
    ) -> Array:
        ...
        if self.parallel:
            ...
            # Attention block.
            x = self.attention(layer_input, inputs_mask, enable_dropout=enable_dropout)
...
class EncoderAndDecoderLayers(nn.Module, param_remapping.ParameterRemappable):
    ...
    def _setup_layers(
        self, module_name: str, num_arguments: int
    ) -> Callable[..., Array]:
        lyrf = utils.maybe_remat(
            self.layer_factory, self.layer_remat, self.scan_layers, static_argnums=(2,)
        )
        logging.info(
        ...
        if self.scan_layers:
            ...
            params_spec = _SCAN_AXIS if initializing else transforms.ScanIn(_SCAN_AXIS)
            cache_spec = 0
            scan_annotation = (
            ...
            lyrf = transforms.factory_scan(
                ...
                data_transform=transforms.inner_scan_spmd(scan_annotation, _SCAN_AXIS),
    ...
    def __call__(
        ...
        *,
        ...
        if decoder_segment_ids is not None or decoder_positions is not None:
            raise ValueError(
                "Packed examples (segment IDs, positions) are not "
    ...
    def __call__(
        ...
        *,
        ...
        if decoder_inputs.ndim != 3:
            raise ValueError(
                "Expect decoder_inputs.ndim=3, but decoder_inputs.ndim="
    ...
    def __call__(
        ...
        *,
        ...
        if prefill or decode or prefill_lengths is not None:
            raise ValueError("Autoregressive cache is not supported by H-Transformer.")
...
class EncoderDecoder(nn.Module, param_remapping.ParameterRemappable):
    ...
    def __call__(
        ...
        *,
        ...
        """
        if (
            encoder_segment_ids is not None
            or decoder_segment_ids is not None
            or (encoder_positions is not None or decoder_positions is not None)
        ):
            raise ValueError(
                "Packed examples (segment IDs, positions) are not "
                "supported by H-Transformer."
