...
import numpy as np
from lit.flaxformer.architectures.common import param_remapping
from lit.flaxformer.architectures.longt5 import long_attention
from lit.flaxformer.architectures.longt5 import longt5_architecture
from lit.flaxformer.architectures.longt5 import (
    relative_position_biases_general,
)
from lit.flaxformer.architectures.t5 import t5_architecture
from lit.flaxformer.components import dense
from lit.flaxformer.components import embedding
from lit.flaxformer.components import layer_norm
from lit.flaxformer.components import relative_position_biases
from lit.flaxformer.components.attention import dense_attention
...
def check_dense_t5_params(
    actual_params: frozen_dict.FrozenDict[str, Any], expected_filename: str
) -> None:
    actual = jax.tree_map(
...
ATTENTION_KERNEL_INIT = nn.initializers.variance_scaling(1.0, "fan_in", "normal")
MLP_KERNEL_INIT = nn.initializers.variance_scaling(1.0, "fan_in", "truncated_normal")
FINAL_KERNEL_INIT = nn.initializers.variance_scaling(1.0, "fan_in", "truncated_normal")
BIAS_INIT = nn.initializers.normal(stddev=1e-6)


class DegenerateLongSelfAttention(
    dense_attention.MultiHeadDotProductAttention, long_attention.LongSelfAttention
):
    ...
    def __call__(
        ...
        *,
        ...
        return super().__call__(
            inputs, inputs, encoder_mask, encoder_bias, enable_dropout=enable_dropout
        )
...
def make_config1(
    scan_layers: bool = False, layer_remat: str = "legacy"
...
def make_config2_shared_relative_position_bias() -> longt5_architecture.LongEncoderDecoder:
    dtype = jnp.float32
    num_attn_heads = 8
    make_dropout = lambda: nn.Dropout(rate=0.1, broadcast_dims=(-2,))
    make_layer_norm = layer_norm.T5LayerNorm
...
# TODO: DRY up with above configs.
def make_config3_shared_token_embedder() -> longt5_architecture.LongEncoderDecoder:
    dtype = jnp.float32
    num_attn_heads = 8
    make_dropout = lambda: nn.Dropout(rate=0.1, broadcast_dims=(-2,))
    make_layer_norm = layer_norm.T5LayerNorm
...
class LongEncoderDecoderTest(absltest.TestCase):
    def test_encoder_shapes_with_relative_attention_per_layer(self):
        ...
        reformatted = transformer.apply(
            {}, variables["params"], method=transformer.to_save_format
        )
        check_dense_t5_params(reformatted, "encoder_shapes_per_layer_relpos_bias.json")
        self.assertEqual(output.shape, (2, 4, 13))

        # Convert back to Flax module structure format and test again.
        params2 = transformer.apply(
            {}, reformatted, method=transformer.from_save_format
        )
    ...
    def test_encoder_example_packing(self):
        ...
        encoder_input_tokens_packed = np.array(
            [[101, 183, 20, 75, 101, 392, 19]], dtype=np.int32
        )
    ...
    def test_entire_transformer_shared_embeds(self):
        ...
        reformatted = transformer.apply({}, params, method=transformer.to_save_format)
        check_dense_t5_params(
    ...
    def test_encoder_local_self_attention_example_packing(self):
        ...
        # Check that the first element matches, which is entire first batch of the
        # padded setup, and the first 3 "tokens" of the packed example.
        np.testing.assert_allclose(output[0, :, :], output_packed[0, 0:4, :], rtol=1e-4)
