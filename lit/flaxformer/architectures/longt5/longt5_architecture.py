...
from lit.flaxformer.components import transforms
...
class LongEncoderLayer(nn.Module, param_remapping.ParameterRemappable):
    ...
    def setup(self):
        if self.relpos_bias_factory is not None and self.shared_relpos_bias is not None:
            raise ValueError(
        ...
        self.relpos_bias = (
            ...
            if self.relpos_bias_factory is not None
            else self.shared_relpos_bias
        )
        if (
            self.side_relpos_bias_factory is not None
            and self.shared_side_relpos_bias is not None
        ):
            raise ValueError(
        ...
        self.side_relpos_bias = (
            self.side_relpos_bias_factory()
            if self.side_relpos_bias_factory is not None
            else self.shared_side_relpos_bias
        )
        ...
        self.attention = self.attention_factory(
            **attention_factory_kwargs
        )  # pytype: disable=wrong-keyword-args  # dict-kwargs
...
class LongEncoder(nn.Module, param_remapping.ParameterRemappable):
    ...
    def setup(self):
        ...
        if self.scan_layers and (
            self.shared_relpos_bias_factory or self.shared_side_relpos_bias_factory
        ):
            raise ValueError(
                "Scanned layer mode doesn't support shared relative" "position biases."
            )
        self.relpos_bias = (
            self.shared_relpos_bias_factory()
            if self.shared_relpos_bias_factory is not None
            else None
        )
        self.side_relpos_bias = (
            self.shared_side_relpos_bias_factory()
            if self.shared_side_relpos_bias_factory is not None
            else None
        )
        ...
        lyrf = lambda: self.layer_factory(
            **layer_kwargs
        )  # pytype: disable=wrong-keyword-args  # dict-kwargs
        ...
        if not self.scan_layers:
            ...
            scan_annotation = (
                ...
                if self.spmd_annotations is not None
                else None
            )
    ...
    def embed_and_combine_inputs(
        self, inputs, inputs_positions=None, *, enable_dropout: bool = True
    ):
    ...
    def __call__(
        ...
        *,
        ...
        embedded_inputs = self.embed_and_combine_inputs(
            inputs, inputs_positions=inputs_positions, enable_dropout=enable_dropout
        )
