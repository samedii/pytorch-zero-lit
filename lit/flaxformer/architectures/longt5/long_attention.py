...
from lit.flaxformer.architectures.longt5 import (
    relative_position_biases_general,
)
from lit.flaxformer.architectures.longt5 import tensor_utils
from lit.flaxformer.components import dense
from lit.flaxformer.components import layer_norm
from lit.flaxformer.components.attention import (
    dense_attention,
)  # GOOGLE-INTERNAL # pylint: disable=line-too-long
from lit.flaxformer.types import Array
from lit.flaxformer.types import DType
from lit.flaxformer.types import Initializer
from lit.flaxformer.types import PRNGKey
...
class EncoderLocalSelfAttention(nn.Module, LongSelfAttention):
    ...
    def __call__(
        ...
        *,
        ...
        validate_long_attention_call_parameter_shapes(
            inputs, inputs_mask, segment_ids, positions
        )
        ...
        if self.relpos_bias:
            ...
            bias = self.relpos_bias(rp_bucket)[
                jnp.newaxis, ...
        ...
        x = _local_self_attention(
            ...
            concat_3_blocks_implementation=self.concat_3_blocks_implementation,
...
def _local_self_attention(
    ...
    attn_weights = (_softmax_with_extra_logit if use_extra_logit else jax.nn.softmax)(
        attn_weights
    ).astype(dtype)
    ...
    y = jnp.einsum("...hqk,...khd->...qhd", attn_weights, value, precision=precision)
...
class EtcTransientGlobalSelfAttention(nn.Module, LongSelfAttention):
    ...
    def __call__(
        ...
        *,
        ...
        # [batch, num_blocks, 1, block_len, 3 * block_len] shape.
        mask = tensor_utils.make_3block_local_att_mask(
            block_len, inputs_mask, segment_ids, use_causal_mask=self.causal
        )[:, :, jnp.newaxis, :, :]
        attention_bias = mask_to_bias(mask, self.dtype)
        ...
        side_mask = jnp.equal(
            segment_ids[..., jnp.newaxis], global_segment_ids[..., jnp.newaxis, :]
        )
        ...
        if self.causal:
            orphans = identify_orphan_tokens(
                self.tokens_per_block, inputs_mask, positions
            )
            ...
            causal_side_mask = jnp.less(
                global_positions, effective_block_ids[..., :, jnp.newaxis]
            )
        ...
        side_bias = self.side_relpos_bias(
            side_rp_bucket
        )  # pylint: disable=not-callable
...
def make_etc_fixed_block_ids(
    ...
    positions = (
        jnp.arange(inputs_mask.shape[-1]) * inputs_mask
    ...
    return vmapped_fn(
        tokens_per_block, inputs_mask, segment_ids, positions, adopt_orphan_tokens
    )
...
def _make_etc_fixed_block_ids_1d(
    ...
    global_start = jnp.logical_and(start_marker, blocks_with_starts[candidate_blocks])

    blocks_without_globals = candidate_block_sums == 0
    token_without_global = blocks_without_globals[candidate_blocks]
    token_without_global = jnp.logical_or(
        token_without_global, jnp.logical_not(inputs_mask)
    )

    block_ids = jnp.cumsum(global_start) * jnp.logical_not(token_without_global) - 1
    ...
    if not adopt_orphan_tokens:
        orphan_tokens = _identify_orphan_tokens(
            tokens_per_block, inputs_mask, positions
        )
...
def identify_orphan_tokens(
    tokens_per_block: int, inputs_mask: Array, positions: Optional[Array] = None
) -> Array:
    ...
    vmapped_fn = jax.vmap(_identify_orphan_tokens, in_axes=(None, 0, 0), out_axes=0)
    return vmapped_fn(tokens_per_block, inputs_mask, positions)


def _identify_orphan_tokens(
    tokens_per_block: int, inputs_mask: Array, positions: Array
) -> Array:
...
def _create_global_aggregates(
    inputs: Array, block_ids: Array, global_seq_len: int
) -> Array:
...
def _local_plus_side_attention(
    ...
    assert (
        query.shape[:-3] == side_key.shape[:-3] == side_value.shape[:-3]
    ), "side k, v batch dims must match q."
    assert (
        query.shape[-2] == side_key.shape[-2] == side_value.shape[-2]
    ), "side k, v num_heads must match q."
    assert (
        side_key.shape[-3] == side_value.shape[-3]
    ...
    if bias is not None:
        ...
        bias = jnp.broadcast_to(
            bias,
            attn_weights.shape[:-4] + (num_blocks, num_heads, block_len, 3 * block_len),
        )
        ...
        attn_weights += jnp.concatenate((bias, side_bias), axis=-1).astype(
            attn_weights.dtype
        )
...
def _make_side_relpos(
    ...
    """
    block_ids, global_segment_ids = make_etc_fixed_block_ids(
        tokens_per_block, inputs_mask, segment_ids, positions, adopt_orphan_tokens=True
    )
    if not adopt_orphan_tokens:
        orphan_locations = identify_orphan_tokens(
            tokens_per_block, inputs_mask, positions
        )
...
def validate_long_attention_call_parameter_shapes(
    ...
    *,
    allow_positions_without_segment_ids: bool = False,
) -> None:
    ...
    if allow_positions_without_segment_ids:
        if positions is None and segment_ids is not None:
            raise ValueError("`positions` must not be None when `segment_ids` is given")
