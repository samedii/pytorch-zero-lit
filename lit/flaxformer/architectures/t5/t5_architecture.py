...
from lit.flaxformer.components import rich_attention_position_scores
...
class DecoderLayer(nn.Module, param_remapping.ParameterRemappable):
    ...
    def get_bias(
        self,
        max_decode_length: Optional[int],
        decode: bool,
        layer_input: Array,
        encoded: Array,
...
class Encoder(nn.Module, param_remapping.ParameterRemappable):
    ...
    def _construct_scanned_encoder(
        self, lyrf: Callable[[], nn.Module], num_layers: int
    ) -> nn.Module:
        ...
        params_spec = (
            self.scan_axis if initializing else transforms.ScanIn(self.scan_axis)
        )
        cache_spec = 0
    ...
    def encode_from_continuous_inputs(
        self, inputs, encoder_mask=None, logit_mask=None, *, enable_dropout: bool = True
    ):
    ...
    def decode_from_continuous_inputs(
        ...
        *,
        ...
        if self.logits_dense is not None:
            ...
            logits = self.embedder.embedders["token_ids"].attend(
                decoder_outputs
    ...
    def __call__(
        ...
        **kwargs,
        ...
        logit_mask = dense_attention.get_decoder_logit_mask(
            decoder_input_tokens, embedded_inputs.dtype
        )
...
def _call_optional(fn: Optional[Callable[[], nn.Module]]) -> Optional[nn.Module]:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if decode and prefill:
            raise ValueError(
                "Only one of `decode` and `prefill` can be set. Use "
