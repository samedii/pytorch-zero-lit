...
from lit.flaxformer.architectures.t5 import (
    t5_architecture_test_utils as t5_test_utils,
)

expected_files = testing_utils.ExpectedJsonFiles("flaxformer/architectures/t5/testdata")
...
class EncoderDecoderTest(absltest.TestCase):
    ...
    def test_parallel_transformer_config(self):
        ...
        expected_files.check_params(
            reformatted, "parallel_transformer_encoder_shapes.json"
        )
        self.assertEqual(output.shape, (2, 4, 13))
    ...
    def test_scan_and_remat(self):
        ...
        transformer1 = t5_test_utils.make_config1(scan_layers=False, layer_remat="none")
        ...
        transformer3 = t5_test_utils.make_config1(scan_layers=False, layer_remat="full")
        ...
        transformer5 = t5_test_utils.make_config1(scan_layers=True, layer_remat="full")
    ...
    def test_capture_input_gradients(self):
        ...
        def fake_loss(
            variables, encoder_input_tokens, decoder_input_tokens, decoder_target_tokens
        ):
            """Returns a loss."""
            output, _ = transformer.apply(
                ...
                enable_dropout=False,
                mutable=["grads"],
            )  # Needed to enable gradient capture.
            return output.sum()

        grad_fn = jax.grad(fake_loss)
        grads_variables = grad_fn(
            variables, encoder_input_tokens, decoder_input_tokens, decoder_target_tokens
        )
