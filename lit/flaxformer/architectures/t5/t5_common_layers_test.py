...
from lit.flaxformer.architectures.t5 import t5_common_layers
...
class T5BaseTest(absltest.TestCase):
    def test_encoder_layer(self):
        ...
        np.testing.assert_equal(
            expected_qkv_shape, np.shape(attention_params["query"]["kernel"])
        )
        np.testing.assert_equal(
            expected_qkv_shape, np.shape(attention_params["key"]["kernel"])
        )
        np.testing.assert_equal(
            expected_qkv_shape, np.shape(attention_params["value"]["kernel"])
        )

        # Validate that the MLP dims are being set appropriately.
        mlp_params = variables["params"]["mlp"]
        np.testing.assert_equal(
            [input_inner_dim, MLP_DIM], np.shape(mlp_params["wi"]["kernel"])
        )
        np.testing.assert_equal(
            [MLP_DIM, input_inner_dim], np.shape(mlp_params["wo"]["kernel"])
        )
    ...
    def test_encoder(self):
        ...
        self.assertContainsSubset(
            ["layers_0", "layers_1", "layers_2"], list(variables["params"].keys())
        )
        ...
        np.testing.assert_equal(
            [EMBEDDING_DIM, MLP_DIM], np.shape(mlp_params["wi"]["kernel"])
        )
        np.testing.assert_equal(
            [MLP_DIM, EMBEDDING_DIM], np.shape(mlp_params["wo"]["kernel"])
        )
    ...
    def test_decoder(self):
        ...
        attention_params = variables["params"]["layers_2"]["encoder_decoder_attention"]
        np.testing.assert_equal(
            expected_qkv_shape, np.shape(attention_params["query"]["kernel"])
        )
        np.testing.assert_equal(
            expected_encoder_kv_shape, np.shape(attention_params["key"]["kernel"])
        )
        np.testing.assert_equal(
            expected_encoder_kv_shape, np.shape(attention_params["value"]["kernel"])
        )
