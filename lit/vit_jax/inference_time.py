...
from lit.vit_jax.configs import models as config_lib
...
def inference_time(config: ml_collections.ConfigDict, workdir: str):
    ...
    assert config.num_classes, f"Expected --config.num_classes={config.num_classes} > 0"
    assert config.image_size, f"Expected --config.image_size={config.image_size} > 0"

    # Build VisionTransformer architecture
    model_config = config_lib.MODEL_CONFIGS[config.model_name]
    model = models.VisionTransformer(num_classes=config.num_classes, **model_config)
    ...
    images = jnp.ones(
        [
            ...
            config.batch // jax.local_device_count(),
            config.image_size,
            config.image_size,
            3,
        ],
    ...
    logging.info(
        "Going to run %d inferences WITHOUT measuring...", config.initial_steps
    )
