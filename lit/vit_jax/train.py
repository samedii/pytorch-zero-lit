...
from lit.vit_jax import checkpoint
from lit.vit_jax import input_pipeline
from lit.vit_jax import models
from lit.vit_jax import momentum_clip
from lit.vit_jax import utils
...
def train_and_evaluate(config: ml_collections.ConfigDict, workdir: str):
    ...
    model_cls = {"ViT": models.VisionTransformer, "Mixer": models.MlpMixer}[
        config.get("model_type", "ViT")
    ]
    ...
    if model_or_filename:
        ...
        if "-" in model_or_filename:
            ...
            logging.info(
                'Selected fillename="%s" for "%s" with final_val=%.3f',
                filename,
                model_or_filename,
                best.final_val,
            )
        pretrained_path = os.path.join(
            config.pretrained_dir, f"{config.model.model_name}.npz"
        )
    else:
        # ViT / Mixer papers
    ...
    lr_fn = utils.create_learning_rate_schedule(
        total_steps, config.base_lr, config.decay_type, config.warmup_steps
    )
    ...
    opt = momentum_clip.Optimizer(
        dtype=config.optim_dtype, grad_norm_clip=config.grad_norm_clip
    ).create(params)
    ...
    for step, batch in zip(
        ...
        if config.progress_every and step % config.progress_every == 0:
            img_sec_core_train = (
                config.batch * (step - lstep) / (time.time() - lt0)
            ) / jax.device_count()
            lt0, lstep = time.time(), step
        ...
        if (config.eval_every and step % config.eval_every == 0) or (
            ...
            lr = float(lr_fn(step))
            logging.info(
                f"Step: {step} "  # pylint: disable=logging-format-interpolation
        ...
        # Store checkpoint.
        if (
            config.checkpoint_every and step % config.eval_every == 0
        ) or step == total_steps:
