...
class DecoderOnlyModel(BaseTransformerModel):
    ...
    def score_batch(
        ...
        if return_intermediates:
            logits, modified_variables = self._compute_logits(
                params=params, batch=batch, dropout_rng=None, mutable=["intermediates"]
            )
...
def compute_weighted_accuracy(
    logits: jnp.ndarray, targets: jnp.ndarray, weights: Optional[jnp.ndarray] = None
) -> Tuple[jnp.ndarray, jnp.ndarray]:
...
def compute_base_metrics(
    ...
    metrics = {
        ...
        "timing/target_tokens_per_second_per_core": metrics_lib.TimeRate.from_model_output(
            numerator=num_tokens / num_devices
        ),
        "nonpadding_fraction": clu_metrics.Average(
