...
import numpy as np
from lit.t5x import checkpoints
from lit.t5x import optimizers
from lit.t5x import partitioning
from lit.t5x import state_utils
from lit.t5x import test_utils
from lit.t5x import train_state as train_state_lib
from lit.t5x import utils
...
def make_train_state_multi_optimizer(
    params: Mapping[str, Any], param_states: Mapping[str, Any], step: int
) -> FlaxOptimTrainState:
            ...
            [
                (
                    traverse_util.ModelParamTraversal(
                        lambda path, _: "kernel" not in path
                    ),
                    optimizers.sgd(0.1),
                ),
            ]
...
def update_train_state_step(
    train_state: FlaxOptimTrainState, step: int
) -> FlaxOptimTrainState:
...
class CheckpointChunkShapeTest(absltest.TestCase):
    def test_simple(self):
        self.assertEqual(
            [4096, 4096], checkpoints._choose_chunk_shape([4096, 4096], 4096 * 4096)
        )

        self.assertEqual(
            [4096, 4096], checkpoints._choose_chunk_shape([8192, 8192], 4096 * 4096)
        )

        self.assertEqual(
            [4096, 2731], checkpoints._choose_chunk_shape([8192, 8193], 4096 * 4096)
        )
...
class CheckpointsTest(parameterized.TestCase):
    def setUp(self):
        ...
        self.train_state = make_train_state(
            ...
            param_states={"bias": np.int32(1), "kernel": np.array([1, 2], np.uint8)},
        )
        self.train_state_multi_optimizer = make_train_state_multi_optimizer(
            ...
            param_states={"bias": np.int32(1), "kernel": None},
        )
        self.default_mesh_axes = make_train_state(
            ...
            param_states={"bias": None, "kernel": None},
        )
    ...
    @mock.patch("jax.host_count")
    @mock.patch("jax.process_index")
    def call_host_multioptimizer_checkpointer(
        self,
        process_index,
        host_count,
        partitioner,
    ...
    def test_get_parameter_infos(self):
        train_state = make_train_state(
            ...
            param_states={"bias": np.int32(1), "kernel": np.array([1, 2])},
            step=np.int32(42),
        )
        ...
        expected_parameter_infos = {
            "state": {
                "step": checkpoints._ParameterInfo(
                    name="state/step",
                    shape=(),
                    ts_spec=None,
                ...
                "param_states": {
                    "bias": checkpoints._ParameterInfo(
                    ...
                    "kernel": checkpoints._ParameterInfo(
            ...
            "target": {
                "bias": checkpoints._ParameterInfo(
                    ...
                    local_chunk_info=partitioning.LocalChunkInfo(
                        slice=(slice(4096, 8192, None), slice(None, None, None)),
                        replica_id=1,
                    ),
                ...
                "kernel": checkpoints._ParameterInfo(
                    ...
                    axes=PartitionSpec(None, "model"),
                ),
            },
        }  # pyformat: disable
        jax.tree_map(
            self.assertEqual,
            checkpointer._get_parameter_infos(),
            expected_parameter_infos,
        )
    ...
    def test_get_multioptimizer_parameter_infos(self):
        ...
        kernel_state_info = checkpointer._get_parameter_infos()["state"][
            "param_states"
        ]["kernel"]
    ...
    def test_all_steps(self):
        ...
        checkpointer = self.call_host_checkpointer(
            0, 1, partitioner, lambda c: c, np.float32, None
        )
        ...
        self.assertSequenceEqual(
            checkpoints.all_steps(checkpointer.checkpoints_dir + "/"), [0, 10, 42, 100]
        )
    ...
    def test_all_latest_step_public(self):
        ...
        self.assertSequenceEqual(checkpoints.all_steps(self.tmp_dir), [0, 10, 42, 100])
    ...
    def validate_restore(
        ...
        for i in range(host_count):
            ...
            bias_slice = partitioner.get_local_chunk_info(
                params["bias"].shape, ("model", None)
            ).slice
            kernel_slice = partitioner.get_local_chunk_info(
                params["kernel"].shape, (None, "model")
            ).slice
            ...
            actual_train_state = self.call_host_checkpointer(
                ...
                lambda c: c.restore(  # pylint: disable=g-long-lambda
                    step=step, lazy_parameters=lazy_parameters
                ),
                np.float32,
            ...
            self.assertEqual(actual_train_state.param_states["kernel"].dtype, np.uint8)
            ...
            np.testing.assert_equal(
                actual_train_state.params["bias"], params["bias"][bias_slice]
            )
            np.testing.assert_equal(
                actual_train_state.params["kernel"], params["kernel"][kernel_slice]
            )
    ...
    def validate_multioptimizer_restore(
        self,
        host_count,
        num_partitions,
        step=42,
        ...
        for i in range(host_count):
            ...
            actual_train_state = self.call_host_multioptimizer_checkpointer(
                ...
                host_count,
                partitioner,
                lambda c: c.restore(step=step),
                np.float32,
                ds_iter if checkpoint_dataset else None,
            )
            actual_optimizer = (
                actual_train_state._optimizer
            ...
            jax.tree_map(
                lambda x: self.assertEqual(x.dtype, expected_restore_dtype),
                actual_optimizer.target,
            )
            self.assertEqual(actual_step, step)
            self.assertEqual(actual_step.dtype, np.int32)
            jax.tree_map(
                np.testing.assert_array_equal, actual_param_states, param_states
            )
            ...
            self.assertTrue(
                all(
                    jax.tree_leaves(
                        jax.tree_map(
                            lambda x: x.dtype == expected_restore_dtype, actual_params
                        )
                    )
                )
            )
            ...
            np.testing.assert_equal(
                actual_params["kernel"], params["kernel"][kernel_slice]
            )
            if checkpoint_dataset:
                # The next value from the restored iterator should equal the
                # replica set id.
    ...
    def validate_save(
        ...
        for i in reversed(range(host_count)):
            ...
            chunk_id_for_shard = partitioner.get_local_chunk_info(
                jnp.ones((num_ds_shards,)), ["data"]
            ).replica_id

            bias_chunk = partitioner.get_local_chunk_info(
                params["bias"].shape, ("model", None)
            )
            kernel_chunk = partitioner.get_local_chunk_info(
                params["kernel"].shape, (None, "model")
            )
            ...
            if i == 0:
                ...
                bias_tspec["kvstore"]["path"] = bias_tspec["kvstore"]["path"].replace(
                    ".tmp-0", ""
                )
                kernel_tspec["kvstore"]["path"] = kernel_tspec["kvstore"][
                    "path"
                ].replace(".tmp-0", "")
            ...
            if checkpoint_dataset:
                ...
                if i != 0:
                    ckpt_dir += ".tmp-0"
                ds_ckpt_glob = gfile.glob(
                    ckpt_dir + "/train_ds-" + f"{ds_shard_id:03}-of-{num_ds_shards:03}*"
                )
            ...
            if bias_chunk.replica_id == 0:
                current_bias = (
                    ts.open(bias_tspec).result().read().result().view(save_dtype)
                )
            ...
            if kernel_chunk.replica_id == 0:
                current_kernel = (
                    ts.open(kernel_tspec).result().read().result().view(save_dtype)
                )
                expected_kernel[kernel_chunk.slice] = params["kernel"][
                    kernel_chunk.slice
                ]
        ...
        self.assertEqual(ckpt_contents["optimizer"]["state"]["step"].dtype, np.int32)
        if disable_partitioning:
    ...
    DTYPES = [jnp.int32, jnp.float32, jnp.bfloat16, jnp.uint32, jnp.int64, jnp.float64]
    ...
    @parameterized.parameters(itertools.product(TOPOLOGIES, TOPOLOGIES))
    def test_multioptimizer_save_multioptimizer_restore(
        self, save_topology, restore_topology
    ):
    ...
    def test_load_t5x_checkpoint(self):
        ...
        jax.tree_map(np.testing.assert_array_equal, self.train_state.state_dict(), ckpt)
    ...
    def test_load_t5x_checkpoint_of_multioptimizer(self):
        ...
        jax.tree_map(
            np.testing.assert_array_equal,
            self.train_state_multi_optimizer.state_dict(),
            ckpt,
        )
        # Jax tree maps ignore Nones so actually check this value is None
    ...
    def test_load_t5x_checkpoint_lazy(self):
        self.validate_save(1, 1)
        ckpt = checkpoints.load_t5x_checkpoint(self.tmp_dir)
        lazy_ckpt = checkpoints.load_t5x_checkpoint(self.tmp_dir, lazy_parameters=True)
    ...
    @parameterized.parameters(TOPOLOGIES)
    def test_save_non_partitioned_restore_non_partitioned(self, *restore_topology):
        # Save without partitioning.
        self.validate_save(2, 1, disable_partitioning=True)
        # Restore with partitioning.
        self.validate_restore(*restore_topology, disable_partitioning=True)
    ...
    def test_keep(self, unused_mock_time):
        ...
        checkpointer = checkpoints.Checkpointer(
            train_state, no_partitions_partitioner, self.tmp_dir, keep=2, use_gda=False
        )
    ...
    def test_keep_pinned(self, unused_mock_time):
        ...
        train_state = self.train_state
        checkpointer = checkpoints.Checkpointer(
            train_state, no_partitions_partitioner, self.tmp_dir, keep=1, use_gda=False
        )
    ...
    def test_keep_dataset_checkpoints_pinned(self, unused_mock_time):
        ...
        # Assert both the pinned and the most recent checkpoints are saved.
        self.assertSequenceEqual(checkpointer.all_steps(), [42, 43])
        self.assertSequenceEqual(checkpointer.all_dataset_checkpoint_steps(), [42, 43])
        ...
        # recent one are still saved.
        self.assertSequenceEqual(checkpointer.all_steps(), [42, 44])
        self.assertSequenceEqual(checkpointer.all_dataset_checkpoint_steps(), [42, 44])
    ...
    def test_keep_with_save_best_checkpointer(self, unused_mock_time):
        ...
        summary_writer = tensorboard.SummaryWriter(os.path.join(self.tmp_dir, "train"))
    ...
    @mock.patch("time.time", return_value=0)
    def test_keep_pinned_save_best_checkpointer_missing_metrics(self, unused_mock_time):
            ...
            lambda c: c.restore(  # pylint:disable=g-long-lambda
                ...
                state_transformation_fns=[
                    functools.partial(
                        ...
                        assignment_map=[
                            ("target/layer2/bias", "target/kernel"),
    ...
    def test_assignment_map_unused(self):
        self.validate_save(1, 1)
        with self.assertRaisesWithLiteralMatch(
            ValueError, "Unused patterns in `assignment_map`: {'target/layer\\d/(.*)'}"
        ):
            self.call_host_checkpointer(
                0,
                1,
    ...
    def test_find_checkpoint(self):
        ...
        self.assertEqual(
            ...
            os.path.join(
                self.fake_checkpoints, f"checkpoint_{self.steps[-1]}", "checkpoint"
            ),
        )
        # `model_dir` with step
        step = 100
        ...
        # checkpoint_file
        path = os.path.join(self.fake_checkpoints, f"checkpoint_{step}", "checkpoint")
    ...
    def test_restore_from_invalid_path(self):
        with self.assertRaisesRegex(
            ValueError, r"Path is not a valid T5X checkpoint: .*"
        ):
            self.verify_restore_checkpoint_from_path(
                TESTDATA, test_utils.get_t5_test_model()
            )
    ...
    def test_save_lazy_optimizer(self):
        ...
        self.call_host_checkpointer(
            ...
            1,
            partitioner,
            lambda c: c.save(train_state=train_state, concurrent_gb=2),
            np.float32,
            None,
        )
        ...
        jax.tree_map(
            np.testing.assert_allclose, train_state.state_dict(), loaded_train_state
        )
            ...
            "optimizer": {
                "target": {
                    ...
                    "sharded_param": {
                        "driver": "zarr",
                        "dtype": "float32",
                        "kvstore": {"driver": "file", "path": "target.sharded_param"},
                        "metadata": {
                            "chunks": [768, 768],
            ...
            "optimizer": {
                "target": {
                    ...
                    "sharded_param": {
                        "driver": "zarr",
                        "dtype": "float32",
                        "kvstore": {"driver": driver, "path": "target.sharded_param"},
                        "metadata": {
                            "chunks": [768, 768],
                            "compressor": {"id": "gzip", "level": 1},
                            "shape": [768, 768],
    ...
    def assert_update_ts_path_from_relative_to_absolute(
        self, ts_spec_dict, expected, ckpt_dir
    ):
        ...
        jax.tree_map(
            np.testing.assert_array_equal, normalized_ts_spec_dict, normalized_expected
        )

        # Test without normalization (corresponds to tensorstore<0.1.14)
        checkpoints._update_ts_path_from_relative_to_absolute(ckpt_dir, ts_spec_dict)
    ...
    def test_update_ts_path_from_relative_to_absolute_gcs(self):
        ts_spec_dict = {
            "driver": "zarr",
            "dtype": "float32",
            "kvstore": {"bucket": "t5x-dummy-bucket", "driver": "gcs"},
            "metadata": {
                "chunks": [768, 768],
                "compressor": {"id": "gzip", "level": 1},
                "shape": [768, 768],
