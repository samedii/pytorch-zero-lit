    ...
    *,
    ...
    def _verify_matching_vocabs(cfg: utils.DatasetConfig):
        ...
        if (
            ...
            raise ValueError(
                f"Model and Task vocabularies do not match:\n"
    ...
    def _run_training_eval(first_run: bool = False):
        ...
        trainer.stop_training = run_actions(
            trainer_lib.ActionMode.TRAIN_EVAL,
            actions,
            trainer.train_state,
            eval_summaries,
        )
    ...
    def _run_inference_eval():
        ...
        all_metrics, _, _ = evaluator.evaluate(
            ...
            predict_fn=functools.partial(
                predict_fn, train_state=trainer.train_state, rng=jax.random.PRNGKey(0)
            ),
    ...
    if checkpoint_period:
        ...
        checkpoint_manager.save(
            trainer.train_state, checkpoint_cfg.save.state_transformation_fns
        )
        ...
        try:
            ...
            while host_step < epoch_end_step:
                if trainer.stop_training:
                    ...
                    logging.info(
                        "Stopping training loop early since `stop_training` is "
                        "requested."
                    )
                    break
...
if __name__ == "__main__":
    ...
    def _main(argv: Sequence[str]):
        ...
        if FLAGS.multiprocess_gpu:
            if (
                FLAGS.coordinator_address is None
                or FLAGS.process_count is None
                or FLAGS.process_index is None
            ):
                raise ValueError(
                    "`coordinator_address`, `process_count` and `process_index` "
