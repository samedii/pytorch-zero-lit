...
class FlaxOptimTrainStateTest(absltest.TestCase):
    ...
    def test_create_with_params_axes(self):
        ...
        optimizer_def = adafactor.Adafactor(
            0.42,
            logical_factor_rules={"vocab": FactorDim.COLUMN, "embed": FactorDim.ROW},
        )
        state = train_state_lib.FlaxOptimTrainState.create(
            optimizer_def, model_variables
        )
    ...
    def test_create_with_flax_mutables_axes(self):
        ...
        self.assertEqual(
            state.flax_mutables, flax.core.freeze({"grads": model_variables["grads"]})
        )
        jax.tree_map(
            np.testing.assert_array_equal, model_variables["params"], state.params
        )
    ...
    def test_replace_params(self):
        ...
        jax.tree_map(
            np.testing.assert_array_equal, expected_state_dict, new_state.state_dict()
        )
