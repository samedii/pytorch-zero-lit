    ...
    def __init__(self, name: str, summary_dir: Optional[str] = None):
        ...
        if jax.process_index() == 0:
            self._writer = metric_writers.create_default_writer(
                summary_dir, collection=name, asynchronous=True
            )
        else:
...
class BaseTrainer(abc.ABC):
    ...
    def compile_eval(self, batches: Mapping[str, BatchType]) -> None:
        ...
        for eval_name, batch in batches.items():
            tick = time.time()
            cache_key: BatchSpec = FrozenDict(
...
def accumulate_grads_microbatched(
    ...
    if num_microbatches is None or num_microbatches <= 1:
        ...
        def per_microbatch_train_step(
            ...
            jnp.ndarray, jnp.ndarray, Mapping[str, jnp.ndarray], Optional[FlaxMutables]
        ]:
...
def eval_step(
    model: models.BaseModel, train_state: train_state_lib.TrainState, batch: jnp.ndarray
) -> MetricMapType:
...
def train_with_lr(
    ...
    grad_accum, metrics, flax_mutables = accumulate_grads_microbatched(
        model, train_state, batch, dropout_rng, num_microbatches, data_partition_spec
    )
...
class EarlyStoppingAction(BaseAction):
    ...
    def run(
        ...
        if not isinstance(m, clu.values.Scalar):
            logging.warning(
                "Metric %s does not have Scalar type. Found %s.", self._metric, type(m)
            )
            _warn_action_not_run(type(self), self._task, self._metric)
            return False
