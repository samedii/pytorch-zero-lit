...
def get_train_state_initializer(
    ...
    input_shapes = {
        k: (p_batch_size, l)
        for k, l in feature_converter.get_model_feature_lengths(
...
def create_decoder_preprocessor(
    ...
    def preprocess(input_texts: tf.Tensor) -> Mapping[str, tf.Tensor]:
        ...
        decoder_loss_weights = tf.cast(
            tf.math.logical_xor(inputs, padding_mask), dtype=decoder_target_tokens.dtype
        )
...
def save(
    *,
    ...
    if not os.path.basename(output_dir).isdigit():
        raise ValueError(
            "output_dir must be in the form ${BASE}/${VERSION}, where "
    ...
    signatures = {
        tf.saved_model.DEFAULT_SERVING_SIGNATURE_DEF_KEY: module.__call__.get_concrete_function(
            *input_signature
        )
    }
