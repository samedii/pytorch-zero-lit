...
from lit.t5x import checkpoint_importer
from lit.t5x import checkpoint_utils
...
class Checkpointer(object):
    ...
    def __init__(
        ...
        dataset_iterator: Optional[
            Union[tf.data.Iterator, clu.data.DatasetIterator]
        ] = None,
        *,
        ...
        *,
        ...
        if jax.process_index() == 0:
            written_state_dict = jax.tree_util.tree_map(
                _get_local_data, written_state_dict
            )
    ...
    def _write_state_to_tensorstore(
        ...
        async def _write_array(
            ...
            if (
                isinstance(maybe_arr, gda_lib.GlobalDeviceArray)
        ...
        for k in state_dict_for_save.keys():
            ...
            future_written_state[k] = jax.tree_util.tree_map(
                functools.partial(
                    _write_array, cast=(k == "target" and self._save_dtype is not None)
                ),
...
class CheckpointManager(orbax.checkpoint.CheckpointManager):
    ...
    def _parameter_infos(self, train_state: train_state_lib.TrainState) -> PyTreeDef:
    ...
    def _get_save_directory(
        self, step: int, directory: epath.Path, key_name: Optional[str] = None
    ) -> epath.Path:
    ...
    def save(
        ...
        save_kwargs = {
            _STATE_KEY: {"save_args": save_args, "tmp_directory": self._tmp_directory},
            _DATASET_KEY: {"tmp_directory": self._tmp_directory},
        }
    ...
    def restore(
        ...
        transforms = _transforms_from_state_transformation_fns(state_transformation_fns)
        ...
        restore_kwargs = {
            _STATE_KEY: {"restore_args": restore_args, "transforms": transforms}
        }
    ...
    def _add_checkpoint_info(self, step, metrics):
        ...
        if jax.process_index() == 0:
            ...
            final_directory = os.fspath(self._get_save_directory(step, self.directory))
