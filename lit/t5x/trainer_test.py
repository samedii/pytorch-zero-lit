...
import numpy as np
from lit.t5x import metrics as metrics_lib
from lit.t5x import models as models_lib
...
from lit.t5x import trainer as trainer_lib
...
class MetricsManagerTest(absltest.TestCase):
    ...
    def test_write_metrics_summary(self):
        ...
        with mock.patch("jax.process_index", return_value=0), mock.patch(
            "time.time", side_effect=[0, 40]  # start_time, end_time
        ), mock.patch(
            "absl.logging.log"
    ...
    def test_timer_concurrency(self):
        ...
        n = 10
        with mock.patch(
            "time.time", side_effect=range(2 * n)  # start_time, end_time
        ), mock.patch(
            "absl.logging.log"
        ):  # avoids hidden calls to time.time()
            for _ in range(n):
...
class TrainerTest(parameterized.TestCase):
    ...
    def _test_train(self, precompile):
        ...
        if precompile:
            with mock.patch(
                "time.time", side_effect=[0, 1]  # compile start, end
            ), mock.patch(
                "absl.logging.log"
            ):  # avoids hidden calls to time.time()
        ...
        num_steps = 2
        with mock.patch(
            "time.time", side_effect=[1, 5]  # start_time, end_time
        ), mock.patch(
            "absl.logging.log"
        ):  # avoids hidden calls to time.time()
        ...
        jax.tree_map(np.testing.assert_equal, trainer.train_state, expected_train_state)
    ...
    def _test_eval(self, precompile):
        ...
        with mock.patch(
            "time.time", side_effect=[1, 5, 5, 8]  # t1 start, t1 end, t2 start, t2 end]
        ), mock.patch(
            "absl.logging.log"
        ):  # avoids hidden calls to time.time()
            trainer.eval(
    ...
    def test_accumulate_grads_microbatched_without_weight_sum_single_batch(self):
        ...
        i = batch["i"].sum()
        expected_grad_accum = jax.tree_map(lambda x: i, self.init_train_state).params
    ...
    @mock.patch("jax.value_and_grad", fake_value_and_grad_fn_without_weight_sum)
    def test_accumulate_grads_microbatched_without_weight_sum_multiple_batches(self):
        batch_iter = self.dataset.as_numpy_iterator()
        batch = next(batch_iter)
        num_micro_batches = 2
        grad_accum, metrics, flax_mutables = trainer_lib.accumulate_grads_microbatched(
