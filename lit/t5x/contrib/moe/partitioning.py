...
from lit.t5x import partitioning as base_partitioning
...
class MoePjitPartitioner(base_partitioning.PjitPartitioner):
    ...
    def get_logical_axes(  # pytype: disable=signature-mismatch  # overriding-parameter-type-checks
        ...
        prepend_expert = (
            lambda x: PartitionSpec(*("expert",) + x)  # pylint: disable=g-long-lambda
            if x
            else PartitionSpec(
                "expert",
            )
...
def compute_num_model_partitions(
    num_model_partitions: Optional[int], model_parallel_submesh: Optional[HardwareMesh]
) -> int:
