...
def _get_index_mappings(device_to_idxs):
    ...
    for d, idx in device_to_idxs.items():
        host_to_idx_set[d.process_index].add(
            gda_lib._hashed_index(idx)
        )  # pylint: disable=protected-access
        idx_to_devices[gda_lib._hashed_index(idx)].add(
            d
        )  # pylint: disable=protected-access
...
class GDADatasetIterator(clu.data.DatasetIterator):
    ...
    def __init__(
        self,
        iterator: clu.data.DatasetIterator,
        partitioner: partitioning.BasePartitioner,
        global_shapes: PyTreeDef,
    ):
    ...
    def __next__(self):
        return _create_gda(self._partitioner, self._global_shapes, next(self._iterator))
    ...
    def iterator(self):
        return (
            self._iterator.iterator
            if isinstance(self._iterator, clu.data.TfDatasetIterator)
            else self._iterator
        )
...
def get_infer_fn(
    ...
    def infer_step_with_indices(params, batch, rng, indices):
        if "rng" in inspect.signature(infer_step).parameters:
            res = typing.cast(InferStepWithRngCallable, infer_step)(params, batch, rng)
        else:
    ...
    def infer_fn(
        ...
        if dataset_remainder:
            ...
            pad_ds = (
                ds.take(1).map(lambda i, x: (np.int64(-1), x)).repeat(dataset_pad_amt)
            )
