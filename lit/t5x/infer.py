...
def create_task_from_tfexample_file(
    ...
    task = seqio.TaskRegistry.add(
        ...
        source=seqio.TFExampleDataSource(
            {"infer": paths}, feature_description=feature_description, reader_cls=reader
        ),
        preprocessors=[
            functools.partial(
    ...
    def infer_task(task: seqio.Task):
        ...
        def _write_chunk_and_canonicalize_ckpt(
            ...
            update_measurement_series(
                "writing_examples_per_sec", chunk, len(inferences) / write_time
            )
        ...
        for chunk, chunk_batch in infer_ds_iter:
            ...
            if jax.process_index() == 0:
                ...
                update_measurement_series(
                    "inference_examples_per_sec", chunk, len(inferences) / chunk_time
                )
